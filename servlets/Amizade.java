...
public class Amizade extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try{
			...
			String visualizado = request.getParameter("visualizado");
			String amizade = request.getParameter("amizade");
			String aceito = request.getParameter("aceito");
			FriendFacade friendFacade = new FriendFacade(centralFacade);
			if(visualizado != null){
				if(visualizado.equals("true")){
					friendFacade.markFriendResponseAsRead(amizade);
				}
			}
			else if(aceito != null){
				if(aceito.equals("true")){
					friendFacade.acceptFriendInvitation(amizade);
				}
				else{
					friendFacade.refuseFriendInvitation(amizade);
				}
			}
