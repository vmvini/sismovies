...
public class CadastroGrupo extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try{
			...
			GroupFacade gf = new GroupFacade(centralFacade);
			String name = request.getParameter("name");
			String descricao = request.getParameter("descricao");
			String meID = String.valueOf( centralFacade.getLoggedUser().getId() );
			gf.createGroup(name, descricao, meID);

			request.setAttribute("grupocadastrado","true");
			goToPage("/criargrupo.jsp", request, response);
